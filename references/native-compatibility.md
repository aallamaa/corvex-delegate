...
The failed child's session metadata recorded `agent_role=corvex_delegate` and `model_provider=openai`; its turn model was `zai-org/GLM-5.2-FP8`. The parent selected the custom role with `fork_turns=none`. Inference failed with HTTP 400 because the GLM model was not supported with ChatGPT account authentication.
...
`scripts/configure_delegate.py check-responses MODEL_ID` probes the API. `scripts/configure_delegate.py install-agent MODEL_ID` modifies user-level Codex configuration and installs a custom-agent file. Use it only when explicitly investigating native integration; it is not part of normal installation. `agent-status` reports configuration presence, not working delegation.
