...
Allow-listed commands run with a fixed environment allow-list (`PATH`, `HOME`, `TMPDIR`, locale, terminal); everything else, including `SSH_AUTH_SOCK`, `PYTHONPATH`, and `CORVEX_API_URL`, is stripped. Configuration flags that make an allow-listed binary run something else are refused, in every spelling that reaches the same option parser: the bare flag, the glued short form (`ssh -oProxyCommand=...`), and a short flag riding in a cluster (`ssh -nNo`). The list covers `git` (`-c`, `-u`, `--exec`, `--exec-path`, `--config-env`, `--git-dir`, `--work-tree`, `--upload-pack`, `--receive-pack`), `ssh` (`-o`, `-F`, `-I`), `rsync` (`-e`, `--rsh`), `find` (`-exec`, `-execdir`, `-ok`, `-okdir`) and `tar` (`-I`, `--to-command`, `--use-compress-program`, `--checkpoint-action`).

Treat that as a speed bump, not a boundary. No flag list can make `git` safe — `git config alias.x '!cmd'` followed by `git x` uses no flag at all — and the same holds for any tool that reads a configuration file the delegate can write. **Allow-listing a command grants everything that command can do**, whether or not it appears above. The environment allow-list and the repository jail are what actually bound the damage.
